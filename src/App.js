...
function App() {
  ...
  */
  ...
  function recenterView() {
    map.getView().animate({ center: geolocation.getPosition() });
  }
...
function drawPosition(event, geolocation) {
  ...
  vectorContext.drawFeature(feature, positionMarkerStyle);
};
...
function drawDestination(event, destination) {
  ...
  var vectorContext = getVectorContext(event);
  var currentPoint = new Point(destination);
  var feature = new Feature(currentPoint);
  vectorContext.drawFeature(feature, destinationMarkerStyle);
};
